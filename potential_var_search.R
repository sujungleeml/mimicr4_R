...
adult_icu <- read.csv("C:/Users/Repository/mimicr4_R/adult_icu.csv")
adult_icu
...
#섬망상태 평가에 따르면... 특성 1 급성 정신상태 변화[Altered LOC] 또는 정신 상태 변동이 심함 => 특성2 주의력 결핍(Inattention) => 특성3 비체계적인사고(disorganized thinking) 또는 특성4 의식수준의 변화 ===> 면섬망으로 진단
...
camicu_rass <- camicu2 %>%  filter(itemid == 228302) %>% 
  rename("rass_lOC" = "value") %>% 
  select("subject_id","hadm_id","stay_id","charttime","itemid","rass_lOC")

camicu_alteredloc <- camicu2 %>%  filter(itemid == 228334) %>% 
  rename("altered_loc"="value")%>% 
  select("subject_id","hadm_id","stay_id","charttime","itemid","altered_loc")

camicu_mschange <- camicu2 %>%  filter(itemid %in% c(228300,228337)) %>% 
  rename("ms_change" = "value") %>% 
  select("subject_id","hadm_id","stay_id","charttime","itemid","ms_change")

camicu_inattention <- camicu2 %>%  filter(itemid %in% c(228301,228336,229325)) %>% 
  rename("inattention" = "value") %>% 
  select("subject_id","hadm_id","stay_id","charttime","itemid","inattention")

camicu_disorgthink <- camicu2 %>%  filter(itemid %in% c(228303,228335,229324)) %>% 
  rename("disorgthink" = "value") %>% 
  select("subject_id","hadm_id","stay_id","charttime","itemid","disorgthink")
...
camicu_tb1 <- camicu_rass %>% full_join(camicu_alteredloc, by = c("subject_id","hadm_id","stay_id","charttime")) %>% 
  ...
  select("subject_id","hadm_id","stay_id","charttime","rass_lOC","altered_loc","ms_change","inattention","disorgthink")
